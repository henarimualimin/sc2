...
# Input Private key
private_key = getpass.getpass(prompt=f"{tata}{kuning}Masukkan private key Anda: {res}")
...
def send_transactions():
    ...
    for address in recipients:
        ...
        print(f"{oke}{hijau}Sukses Kirim: {res}{tx_hash.hex()} {hijau}ke {res}{address} {hijau}for {res}{amount_input} {hijau}{'tokens' if send_token else 'ETH/MATIC/etc'}{res}")
        print(grs)
        time.sleep(2)
